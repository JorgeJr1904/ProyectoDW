document.addEventListener("DOMContentLoaded", function() {
    ...
    if (employId) {
        ...
        fetch(`http://206.189.200.204:8080/api/v1/employee/${employId}`)
            ...
            .then(empleado => {
              ...
              // Actualiza los elementos HTML con los detalles del producto
                ...
                document.getElementById("name_emp").value = empleado.name;
                document.getElementById("last_emp").value = empleado.lastname;
                document.getElementById("telef").value = empleado.tel;
                document.getElementById("direcc").value = empleado.address;
                document.getElementById("email1").value = empleado.email;
                document.getElementById("blood_emp").value = empleado.blood;
                document.getElementById("gen_emp").value = empleado.sex;
                document.getElementById("job_emp").value = empleado.jobTitleId;
...
function save_mod_emple_input(){
  ...
  if (empid) {
      ...
      const last_emp = document.getElementById("last_emp").value;
      ...
      const gen_emp = document.getElementById("gen_emp").value;
      const job_emp = document.getElementById("job_emp").value;
...
//"2023-13-01", fecha json
      ...
      fetch(`http://206.189.200.204:8080/api/v1/employee/update/${empid}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
          },
...
fetch(`http://206.189.200.204:8080/api/v1/employee/delete/${employ_del}`, {
method: 'DELETE',
headers: {
  'Content-Type': 'application/json'
}
...
.then(respuesta => {
if (!respuesta.ok) {
  throw new Error('El elemento ya no existe o la conexión esta caída');
}
return respuesta.json();
})
.then(respuestaJSON => {
...
function gotomain(enlace) {
  ...
  const name_emp = document.getElementById('name_emp');
  const last_emp = document.getElementById('last_emp');    
  ...
  const gen_emp = document.getElementById('gen_emp');
  const job_emp = document.getElementById('job_emp');


  alert("Ya puede modificar los campos");

// Habilita el input
name_emp.disabled = false;
name_emp.style.backgroundColor = 'lightblue';
name_emp.style.border = '2px solid red';

last_emp.disabled = false;
last_emp.style.backgroundColor = 'lightblue';
last_emp.style.border = '2px solid red';
...
gen_emp.disabled = false;
gen_emp.style.backgroundColor = 'lightblue';
gen_emp.style.border = '2px solid red';


job_emp.disabled = false;
job_emp.style.backgroundColor = 'lightblue';
job_emp.style.border = '2px solid red';
